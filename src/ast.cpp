                ...
                if (!decl->children.empty())
                {
                    ...
                    else if (sub->label == "<function-declaration>")
                    {
                        ...
                        for (auto &child : sub->children)
                        {
                            ...
                            {
                                if (!colonFound)
                                    funcName = extractValue(child->label);
                                else
                                    retType = extractValue(child->label);
                            }
...
std::shared_ptr<StmtNode> buildStatement(std::shared_ptr<ParseNode> node)
{
    ...
    else if (actualStmt->label == "<if-statement>")
    {
        if (actualStmt->children.size() >= 4)
        {
            ...
            else
            {
                ...
                if (stmtAst)
                    trueBlock->statements.push_back(stmtAst);
            }
            ...
            if (actualStmt->children.size() >= 6)
            {
                ...
                else
                {
                    ...
                    if (stmtAst)
                        falseBlock->statements.push_back(stmtAst);
                }
            }
    ...
    else if (actualStmt->label == "<case-statement>")
    {
        if (actualStmt->children.size() >= 4)
        {
            ...
            {
                if (!caseBlock || caseBlock->label != "<case-block>")
                    return;
                ...
                if (!branch->labels.empty() || branch->statement)
...
static std::shared_ptr<VarNode> buildVariable(std::shared_ptr<ParseNode> node)
{
    if (!node || node->children.empty())
        return std::make_shared<VarNode>("unknown_var");
    auto var = std::make_shared<VarNode>(extractValue(node->children[0]->label));
    for (size_t i = 1; i < node->children.size(); ++i)
    {
        auto comp = node->children[i];
        if (!comp || comp->label != "<component-variable>" || comp->children.empty())
            continue;
...
static std::shared_ptr<ExprNode> buildConstant(std::shared_ptr<ParseNode> node)
{
    ...
    if (node->label == "<constant>")
    {
        ...
        if (!node->children.empty())
            return buildConstant(node->children.back());
    }
...
static std::string buildTypeText(std::shared_ptr<ParseNode> node)
{
    ...
    if (node->label == "<type>" && !node->children.empty())
        return buildTypeText(node->children[0]);
    if (node->label.find("ident(") == 0)
        return extractValue(node->label);
    if (node->label == "<range>")
        return "subrange";
    if (node->label == "<enumerated>")
        return "enumerated";
    if (node->label == "<record-type>")
        return "record";
...
static std::string quotedLiteralForAST(const std::string &value)
{
...
std::string formatExpr(std::shared_ptr<ExprNode> node, const std::string &baseIndent)
{
    ...
    if (auto v = std::dynamic_pointer_cast<VarNode>(node))
    {
        ...
        for (const auto &selector : v->selectors)
            name += selector;
        return "Var('" + name + "')";
    }
    ...
    if (auto s = std::dynamic_pointer_cast<StringNode>(node))
        return "String(" + quotedLiteralForAST(s->value) + ")";
...
void printSection(std::ostream &os, const std::string &title,
                  const std::string &prefix, bool isLastTitle)
{
    os << prefix << (isLastTitle ? "└── " : "├── ") << title << "\n";
}

void printChild(std::ostream &os, const std::string &prefix,
                const std::vector<std::string> &lines, bool isLast)
{
    if (lines.empty())
        return;
