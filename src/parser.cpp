...
static std::shared_ptr<ParseNode> parseAssignmentStatement(std::shared_ptr<ParseNode> identNode);
...
static const Token &peekAt(int offset = 1) {
    ...
    if (idx < g_tokens.size()) return g_tokens[idx];
    static Token eof{TOKEN_EOF, "", 0, 0};
    return eof;
}
...
static std::shared_ptr<ParseNode> parseStatement() {
    if (check(TOKEN_IDENT)) {
        ...
        if (check(TOKEN_BECOMES))
            return parseAssignmentStatement(identNode);
        else
