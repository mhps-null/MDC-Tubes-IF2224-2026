...
private:
    ...
    ExprInfo visitCall(const std::shared_ptr<ParseNode> &node)
    {
        ...
        ast->annotations = {"tab_index:" + std::to_string(idx),
                            "lev:" + std::to_string(symbols.lexicalLevel())};
            ...
            if (ParseTreeView::node(child, "<parameter-list>"))
                for (const auto &arg : child->children)
                    if (ParseTreeView::node(arg, "<expression>"))
                    {
                        ...
                        ast->children.push_back(args.back().node);
                    }
    ...
    ExprInfo constantInfo(const std::shared_ptr<ParseNode> &node)
    {
        ...
        if (tag == "string")
        {
            ...
            type.stringLength = static_cast<int>(value.size());
            return {type, annotated("String(" + value + ")", {"type:string", "len:" + std::to_string(type.stringLength)}),
                    0, true, 0, value};
        }
        if (tag == "ident")
    ...
    ExprInfo identifierInfo(const std::string &name, bool warnUninitialized = true)
    {
        ...
        if (!idx)
        {
            ...
            return {unknownType(), annotated("Var(" + name + ")",
                                             {"tab_index:0", "type:unknown", "lev:" + std::to_string(symbols.lexicalLevel())}),
                    0, false, 0, ""};
        }
