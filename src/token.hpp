// token.hpp
// Definisi semua tipe token buat bahasa Arion.
// Ada enum TokenType (52 token + ERROR + EOF), struct Token,
// sama fungsi buat konversi token ke string.
...
// semua tipe token sesuai spesifikasi Arion
enum TokenType {
    // literal
...
// struct Token: nyimpen tipe, nilai, sama posisinya di source code
struct Token {
    TokenType type;
    std::string value;
    int line;
...
// konversi TokenType ke string buat output
// format sesuai spesifikasi (contoh: "programsy", "ident", "intcon")
...
// cek apakah token ini perlu ditampilkan valuenya
// yang punya value: intcon, realcon, charcon, string, ident, sama error
...
// format token jadi string buat output
